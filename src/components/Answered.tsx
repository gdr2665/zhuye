...
function Answered(props: propsType) {
  ...
  useEffect(() => {
    ...
    const newMarkerMessage = markerMessage.concat([
      'int 除以 int 结果还是 int，所以结果就错误了。你把 a 和 b 都乘 1.0 试试？',
    ])
          ...
          <Space direction={'vertical'} size={'medium'} className={'problem-right-child'}>
            <Space className={'vertical-center'} size={0}>
              <Space direction={'vertical'} size={0} style={{ width: '165px' }}>
                <$.Name>{props.answerer}</$.Name>
                <$.SecondaryInfo>{props.answererFrom}</$.SecondaryInfo>
              </Space>
              <$.Nbsp width={'3'} />
              <Button
                variant={'text'}
